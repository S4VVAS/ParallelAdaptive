...
implementations = ['PARALLEL_ADAPTIVE_LIST', 'PARALLEL_ADAPTIVE_MAP', 'WRAPPED_MAP', 'WRAPPED_LIST']
names = {'PARALLEL_ADAPTIVE_LIST': 'ParellelAdaptiveList',
         'PARALLEL_ADAPTIVE_MAP': 'ParallelAdaptiveMap',
         'WRAPPED_MAP':'ConcurrentHashMap',
         'WRAPPED_LIST':'CopyOnWriteArrayList'}
combined = {'ADAPTIVE' : ['PARALLEL_ADAPTIVE_LIST', 'PARALLEL_ADAPTIVE_MAP'],
          'WRAPPED' : ['WRAPPED_MAP', 'WRAPPED_LIST']}
props = {'PARALLEL_ADAPTIVE_LIST': ['dashed', 'blue'],
         'PARALLEL_ADAPTIVE_MAP': ['dashed', 'red'],
    ...
    for test in tests:
        for merged in ['ADAPTIVE', 'WRAPPED']:
            ...
            selectedOA.groupby('Threads').mean().plot(kind='line', marker='+', y='Score', ax=ax,
                                                      label='Average ParallelAdaptive',
                                                      color='chocolate')
